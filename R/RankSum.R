...
{ 
   ...
   for (l in 1:num.ori)
    ...
    rank.sum=apply(rank.all,1,sum)
    rank.ave=rank.sum/num.rank.all
    rank.ave[num.rank.all==0]=NA ## genes with all value missing, assigned to NA

    list(RS=rank.ave,rank.all=rank.all)
}
