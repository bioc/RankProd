...
{
  cat("Using memory-efficient version of RPadvance\n");
  ...
  if (!is.null(y) ){   ##two-sample
      ...
      { data1=as.matrix(data1.all[[l]])
        ...
        fold.change = cbind(fold.change,fcoperator(data1.ave,data2.ave));
	##
   ...
   if ( is.null(y)){ ##one-sample
    ...
    RP.ori.out.upin2=RankProd2V2(data1.all,data2.all,num.ori,num.gene,logged,num.class,rev.sorting=FALSE)
    RP.ori.upin2=RP.ori.out.upin2$RP
    rank.ori.upin2=rank(RP.ori.upin2)
    ##study down-regulated genes under condition2
    RP.ori.out.downin2=RankProd2V2(data1.all,data2.all,num.ori,num.gene,logged,num.class,rev.sorting=TRUE)
...
##
    ...
    ##permutation
## fb: Matrix of permutations is no longer completely kept in memory but computed on demand inside the num.perm loop
...
## fb: This is the new column wise iterative process: Ranks are updated right away, no permuted columns need be stored
       RP.perm.upin2.oneCol <- RankProd2V2(new.data1.all,new.data2.all,num.ori,num.gene,logged,num.class,rev.sorting=FALSE)$RP
       RP.perm.downin2.oneCol <- RankProd2V2(new.data1.all,new.data2.all,num.ori,num.gene,logged,num.class,rev.sorting=TRUE)$RP

       temp2.up <- updateRank( temp2.up, RP.ori.upin2, RP.perm.upin2.oneCol )
       temp2.down <- updateRank( temp2.down, RP.ori.downin2, RP.perm.downin2.oneCol )

       rm(new.data.temp,new.data1.all ,new.data2.all,RP.perm.upin2.oneCol,RP.perm.downin2.oneCol )
##
...
## fb: We don't have the matrix of all permuted values... and we already have the ranked matrix
...
##
   ...
   list(pfp=pfp,pval=pval,RPs=RPs,RPrank=RPrank,Orirank="Orirank was not computed (huge=TRUE in call to RP)",AveFC=ave.fold.change,all.FC=fold.change)
